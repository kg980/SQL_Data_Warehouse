...
SELECT -- give friendly names
	...
	CASE 
		...
		ELSE COALESCE(ca.gen, 'N/A') -- fall back on ERP, and set to 'N/A' IF Null (using Coalesce to replace)
	END AS gender,
	ca.bdate AS birthdate,
	ci.cst_create_date AS create_date
FROM silver.crm_cust_info ci
LEFT JOIN silver.erp_cust_az12 ca
	ON ci.cst_key = ca.cid -- prepared these in silver layer, so I know they will match.
...
LEFT JOIN silver.erp_px_cat_g1v2 pc
	ON pn.cat_id = pc.id
WHERE pn.prd_end_dt IS NULL; -- Filter out all historical data (current data has NULL end date)
GO


--- Sales Fact Table from the product and customer dimension tables
...
SELECT 
	...
	pr.product_key, -- using this rather than sls_prd_key as it matches the surrogate key I'm joining on
	cu.customer_id, -- using this rather than sls_cust_id, as it matches the surrogate key I'm joining on
